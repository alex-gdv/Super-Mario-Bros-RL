"""
Source:
https://github.com/jiseongHAN/Super-Mario-RL.git
"""
...
from eval import eval_smb_model
...
def train(smb_version="v0", movement=SIMPLE_MOVEMENT):
    n_frame = 4
...
def eval(model_name, movement=SIMPLE_MOVEMENT, **kwargs):
    n_frame = 4
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    q = model(n_frame, len(movement), device).to(device)
    q.load_state_dict(torch.load(f"{model_name}_q_target.pth"))
    eval_smb_model(q, model_name=model_name, movement=movement, **kwargs)
